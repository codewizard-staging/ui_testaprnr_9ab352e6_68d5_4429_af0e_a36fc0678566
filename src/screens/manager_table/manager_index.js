...
const columns = [
    { headerName: "MId", field: "MId", flex: 1 },
    { headerName: "FullName", field: "FullName", flex: 1 },
    { headerName: "ContactNumber", field: "ContactNumber", flex: 1 },
    { headerName: "Address", field: "Address", flex: 1 },
    { headerName: "SecondaryContactNumber", field: "SecondaryContactNumber", flex: 1 },
...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetManagersCount(query)
            .then(async (res) => {
                if (res.status) {
        ...
        await Api.GetManagersMulti(query, "")
            .then(async (res) => {
                if (res.status) {
                    _rows = res.values || [];
                    if (_rows.length > 0) {
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetManagerSingle({ MId: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/Managers/edit/${id}`;
        if (type === 'view') _route = `/Managers/view/${id}`;
    ...
    return (
        ...
        <>
            <Container {...props}>
                <Box style={{ width: '100%', paddingBottom: 5 }}>
                    ...
                    <Stack direction="row">
                        <Grid container sx={{ justifyContent: 'flex-end' }}>
                                ...
                                onClick={() => NavigateTo("/Managers/create")}
                            >
                                <AddBoxIcon />
                            </IconButton>
                        </Grid>
                ...
                <Box style={{ width: '100%' }}>
                    <DataTable keyId={'MId'} columns={columns} rowsCount={rowsCount} rows={rows} sortBy={sortBy} pageInfo={pageInfo} onActionClicked={OnActionClicked}
