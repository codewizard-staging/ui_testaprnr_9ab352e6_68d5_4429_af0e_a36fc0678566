...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], managerId, manager, numfields;

        manager = row['manager'];
        managerId = row['manager'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) managerId = rslt.id;
                            ...
                            rslt = await Support.AddOrUpdateManager(data, dropDownOptions);
        ...
        global.AlertPopup("success", "Manager is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
