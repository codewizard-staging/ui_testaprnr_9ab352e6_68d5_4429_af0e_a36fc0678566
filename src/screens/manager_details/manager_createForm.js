...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, managerId, numfields;
        const mapItems = MapItems;

        let manager = row['manager'];

        numfields = Helper.GetAllNumberFields(manager);
        if (numfields.length > 0) Helper.UpdateNumberFields(manager, numfields);


        // Add Or Update Manager
        rslt = await Support.AddOrUpdateManager(manager, []);
        if (rslt.status) {
            managerId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the Manager and navigation entity if it is deos not exist
            let navItem = manager.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "MId", value: parseInt(managerId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdateManager(data);
        ...
        global.AlertPopup("success", "Manager is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
